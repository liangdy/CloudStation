...
public class DownloadManager {
    ...
    private Map<FileDownloadTask, OnDownloadingListener> mDowndloadingMap = Collections
            .synchronizedMap(new HashMap<FileDownloadTask, OnDownloadingListener>());
    private Map<FileDownloadTask, OnDownloadProgressListener> mProgressMap = Collections
            .synchronizedMap(new HashMap<FileDownloadTask, OnDownloadProgressListener>());
     ...
     */
    private Map<Integer, String> mCacheKeysForProgressAwares = Collections
    ...
    private boolean isTaskExists(String id, String url) {
        ...
        for (FileDownloadTask task : mTaskList) {
            ...
            if (id.equals(downloadInfo.getId()) &&
                    url.equals(downloadInfo.getUrl())) {
                return true;
            }
        }
        return false;
    ...
    public void downloadFile(String fileName, String id, String url, OnDownloadingListener downloadingListener) {
        downloadFile(fileName, id, url, downloadingListener, null);
    }


    public void downloadFile(String fileName, String id, String url, OnDownloadingListener downloadingListener, OnDownloadProgressListener downloadProgressListener) {
        downloadFile(fileName, id, url, null, downloadingListener, downloadProgressListener);
    }
     ...
     * @param url 下载地址
     */
    public void downloadFile(String fileName, String id, String url, ProgressAware progressAware, OnDownloadingListener downloadingListener, OnDownloadProgressListener downloadProgressListener) {
        checkConfiguration();
        synchronized (mTaskList) {
            ...
            File cacheFile = generateCacheFile(fileName);
            ...
            if (progressAware != null) {
                prepareUpdateProgressTaskFor(progressAware, downloadInfo
                        .getId());
            }
    ...
    public void prepareUpdateProgressTaskFor(ProgressAware progressAware, String fileDownloadInfoId) {
        mCacheKeysForProgressAwares
                .put(progressAware.getId(), fileDownloadInfoId);
    }
    ...
    private File generateCacheFile(String url, int type) {
        ...
        if (type == FileType.TYPE_AUDIO) {
            cacheDir = new File(
                    cacheDir.getAbsolutePath() + File.separator + "audio");
        } else if (type == FileType.TYPE_VIDEO) {
            cacheDir = new File(
                    cacheDir.getAbsolutePath() + File.separator + "video");
        } else if (type == FileType.TYPE_IMAGE) {
            cacheDir = new File(
                    cacheDir.getAbsolutePath() + File.separator + "image");
        } else {
    ...
    private File generateCacheFile(String name) {
        File cacheDir = mDownloadConfiguration.getCacheDir();
        if (!cacheDir.exists()) {
            cacheDir.mkdirs();
        }
    ...
    private OnDownloadingListener mOnDownloadDispatcher = new OnDownloadingListener() {
        ...
        public void onDownloadFailed(final FileDownloadTask downloadInfo, final int errorType, final String msg) {
            final OnDownloadingListener downloadingListener = mDowndloadingMap
                    .remove(downloadInfo);
            ...
            if (downloadingListener != null) {
                if (downloadInfo.isSyncLoading()) {
                    downloadingListener
                            .onDownloadFailed(downloadInfo, errorType, msg);
                } else {
            ...
            if (downloadingListener != null) {
                if (downloadInfo.isSyncLoading()) {
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            downloadingListener
                                    .onDownloadSucc(downloadInfo, outFile);
                        }
                    });
                }
            }
    ...
    private OnDownloadProgressListener mOnDwonloadProgressDispatcher = new OnDownloadProgressListener() {
        ...
        public void onProgressUpdate(final FileDownloadTask fileDownloadInfo, final long current, final long totalSize) {
            final OnDownloadProgressListener progressListener = mProgressMap
                    .get(fileDownloadInfo);
            if (progressListener != null) {
                long t = totalSize;
                if (t == 0) {
                    t = 1;
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        progressListener
                                .onProgressUpdate(fileDownloadInfo, current, totalSize);
                    }
                });
            }
        }
